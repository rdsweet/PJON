...
PJON implements a totally(<-- replace with "total") software emulated network protocol stack in around 1500 lines of code,(<--- replace with ".[period]) ("Thanks" --> capitalize) thanks to the [strategies](/src/strategies/README.md) abstraction it can work nominally with any data link (add "used." --->) is in use ("And" capitalize-->) and can be applied in a variety of media and applications. This library can be cross-compiled (`-std=c++11`) to work with any Arduino compatible device, linux or windows x86 machine.
...
This library exposes to (<--delete "to") users (add "to") a set of methods to easily configure and handle communication between devices:
