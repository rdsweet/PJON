...
PJON implements a totally (<-- change to "total") software emulated network protocol stack in around 1500 lines of code,(<-- replace with ".") thanks (<-- capitalize "Thanks") to the [strategies](/src/strategies/README.md) abstraction it can work nominally with any data link is (<--delete "is") in use and can be applied in a variety of media and applications. This library can be cross-compiled (`-std=c++11`) to work with any Arduino compatible device, linux or windows x86 machine.
